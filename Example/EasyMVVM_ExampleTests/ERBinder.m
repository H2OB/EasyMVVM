...
describe(@"EZMBinder test", ^{
    ...
    context(@"two way binding", ^{
        it(@"should cancel two node's link after self dealloc", ^{
            EZRMutableNode *node1 = [[EZRMutableNode new] named:@"node1"];
            EZRMutableNode *node2 = [[EZRMutableNode new] named:@"node2"];
            @autoreleasepool {
                EZMBinder *binder = [[EZMBinder alloc] init];
                ...
                // Node: mainThreadNode is an inner node in two way bingding.
                //
                //   ┌─────────────┐
                //   ↓             │
                // node2 -> (mainThreadNode) <===> node1
                //
                expect(node1.downstreamTransforms.firstObject.to).to(equal(node2.upstreamNodes.firstObject));
                expect(node2.downstreamTransforms.firstObject.to).to(equal(node1.upstreamNodes.firstObject));
            }

            expect(node1.downstreamNodes.count).to(equal(0));
            expect(node2.downstreamNodes.count).to(equal(0));
        });
        
        it(@"should cancel two node's link after invoking self '- (void)cancel' method", ^{
            EZMBinder *binder = [[EZMBinder alloc] init];
            EZRNode *node1 = [[EZRNode new] named:@"node1"];
            EZRNode *node2 = [[EZRNode new] named:@"node2"];
            [binder twoWayBindNode:node1 toNode:node2];
            expect(node1.downstreamTransforms.firstObject.to).to(equal(node2.upstreamNodes.firstObject));
            expect(node2.downstreamTransforms.firstObject.to).to(equal(node1.upstreamNodes.firstObject));
            [binder cancel];
        ...
        it(@"should keep two node existing after linking two nodes", ^{
            ...
            @autoreleasepool {
                EZMBinder *binder = [[EZMBinder alloc] init];
                weakBinder = binder;
                EZRNode *node1 = [[[EZRNode alloc] init] named:@"node1"];
                EZRNode *node2 = [[[EZRNode alloc] init] named:@"node2"];
                weakNode1 = node1;
                weakNode2 = node2;
    ...
    context(@"Memory managerment", ^{
        fit(@"should keep existing after initialized with associate object", ^{
            EZRMutableNode *node1 = [[EZRMutableNode new] named:@"node1"];
            EZRMutableNode *node2 = [[EZRMutableNode new] named:@"node2"];
            EZRMutableNode *node3 = [[EZRMutableNode new] named:@"node3"];
            EZRMutableNode *node4 = [[EZRMutableNode new] named:@"node4"];

            NSObject *owner = nil;
            __weak EZMBinder *binder1 = nil;
            __weak EZMBinder *binder2 = nil;
            @autoreleasepool {
                owner = [NSObject new];
                binder1 = [EZMBinder binderWithAssociateObject:nil];
                binder2 = [EZMBinder binderWithAssociateObject:owner];
                [binder1 bindNode:node1 toNode:node2];
                [binder2 bindNode:node3 toNode:node4];
            }
            
            expect(binder1).to(beNil());
            expect(binder2).notTo(beNil());
            expect(node1.upstreamNodes.count).to(equal(0));
            expect(node3.upstreamNodes.count).notTo(equal(0));

            owner = nil;
            expect(binder2).to(beNil());
            expect(node3.upstreamNodes.count).to(equal(0));
        });
    });
});
