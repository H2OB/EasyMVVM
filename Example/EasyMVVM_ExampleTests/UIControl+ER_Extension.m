...
describe(@"UIControl extension test", ^{
    context(@"UIControl extension", ^{
        ...
        it(@"can listen to each event from an UIControl", ^{
            ...
            [[value listenedBy:listener] withListenTransform:listenTransform];
            
            [button sendActionsForControlEvents:UIControlEventTouchUpInside];
            [button sendActionsForControlEvents:UIControlEventTouchDown];
            [button sendActionsForControlEvents:UIControlEventTouchUpInside];
            
            expect(listenTransform.receiveValues).to(haveCount(2));
        });
        ...
        it(@"won't be released if there is at least one listener", ^{
            ...
            @autoreleasepool {
                ...
                @autoreleasepool {
                    ...
                    value = strongValue;
                    cancelable = [[value listenedBy:button] withBlock:^(EZMControlWithEvents *next) {
        ...
        it(@"will be released if the UIControl is released", ^{
            expectCheckTool(^(CheckReleaseTool *checkTool) {
                ...
                [[value listenedBy:button] withBlock:^(id next) {
    ...
    context(@"UIBarButtonItem+EREvent", ^{
...
#pragma clang diagnostic pop
    ...
    context(@"Extension for UIControls", ^{
        it(@"can get the selected index of a UISegmentedControl through an EZRNode", ^{
            ...
            EZRMutableNode<NSNumber *> *index = segControl.ezm_selectedIndex;
            index.value = @2;
        ...
        it(@"can get the value of a UISlider through an EZRNode", ^{
            ...
            EZRMutableNode<NSNumber *> *slidEZRNode = slider.ezm_value;
            slider.value = 0.5;
            expect(slidEZRNode.value).to(equal(@0.5));
            slidEZRNode.value = @0.3;
            expect(slider.value).to(beCloseTo(@0.3));
        });
        
        it(@"can get the status of a UISwitch through an EZRNode", ^{
            UISwitch *sw = [[UISwitch alloc] init];
            EZRMutableNode<NSNumber *> *switchValue = sw.ezm_on;
            expect(switchValue.value.boolValue).to(beFalse());
            switchValue.value = @YES;
        ...
        it(@"can get the value of a UIStepper through an EZRNode", ^{
            ...
            EZRMutableNode<NSNumber *> *steppEZRNode = stepper.ezm_value;
            expect(steppEZRNode.value).to(equal(@0));
            stepper.value = 42;
            expect(steppEZRNode.value).to(equal(@42));
            steppEZRNode.value = @99;
            expect(stepper.value).to(equal(99));
        });
        
        it(@"can get the value of a UILabel through an EZRNode", ^{
            UILabel *label = [[UILabel alloc] init];
            EZRMutableNode<NSString *> *labeelNode = label.ezm_text;
        ...
        it(@"can get the current page of a UIPageControl through an EZRNode", ^{
            ...
            EZRMutableNode<NSNumber *> *pageValue = pageControl.ezm_currentPage;
        ...
        it(@"can get the current text of a UITextField through an EZRNode", ^{
            ...
            EZRMutableNode<NSString *> *textValue = tf.ezm_text;
        ...
        it(@"returns the same node if it was called multiple times", ^{
            ...
            EZRNode *slidEZRNode1 = slider.ezm_value;
            EZRNode *slidEZRNode2 = slider.ezm_value;
            expect(slidEZRNode1 == slidEZRNode2).to(beTrue());
            ...
            EZRNode *steppEZRNode1 = st.ezm_value;
            EZRNode *steppEZRNode2 = st.ezm_value;
            expect(steppEZRNode1 == steppEZRNode2).to(beTrue());
        ...
        xit(@"can be released correctly", ^{
            ...
            
            {
                expectCheckTool(^(CheckReleaseTool *checkTool) {
                    ...
                    [checkTool checkObj:slidEZRNode];
                }).to(beReleasedCorrectly());
            }
            ...
            
            {
                expectCheckTool(^(CheckReleaseTool *checkTool) {
                    ...
                    [checkTool checkObj:steppEZRNode];
                }).to(beReleasedCorrectly());
            }
            
            {
                expectCheckTool(^(CheckReleaseTool *checkTool) {
                    UILabel *label = [[UILabel alloc] init];
                    EZRMutableNode<NSString *> *labelNode = label.ezm_text;
                    label.text = @"xx";
                    labelNode.value = @"zz";
